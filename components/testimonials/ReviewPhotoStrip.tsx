  ...
  return (
    <div className="mt-4 flex items-end gap-2 overflow-hidden">
      {visiblePhotos.map((url, index) => (
        <div
          key={url}
          className={`relative overflow-hidden rounded-xl bg-muted ring-1 ring-border/60 ${
      ...
      {hiddenPhotosCount > 0 ? (
        <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-foreground text-xs font-semibold text-background">
          +{hiddenPhotosCount}
        </div>
      ) : null}
    </div>
