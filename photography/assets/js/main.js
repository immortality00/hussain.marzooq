  ...
  var _ = {
    ...
    initEvents: function() {
      ...
      _.$viewer.on("touchend", function() {
        if (breakpoints.active("<=medium")) _.hide();
      });
    ...
    init: function() {
      ...
      breakpoints.on(">xsmall", function() {
        if (_.current === null) _.switchTo(0, true);
      });
    },
    ...
    switchTo: function(index, noHide) {
      ...
      if (_.current == index && !breakpoints.active("<=xsmall")) return;
      ...
      if (!noHide && breakpoints.active("<=medium")) _.hide();
      ...
      else window.setTimeout(f, _.settings.slideDuration);
    },
     ...
     */
    toggle: function() {
      if (_.$body.hasClass("fullscreen")) _.show();
      else _.hide();
    }
  };
  return _;
